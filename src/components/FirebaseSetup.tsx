...
import { firebaseService, FirebaseConfig } from '@/lib/firebase'
...
export default function FirebaseSetup() {
  ...
  const [firebaseConfig, setFirebaseConfig] = useState<Partial<FirebaseConfig>>({})
  ...
  const initializeFirebase = async () => {
    ...
    try {
      ...
      const initialized = firebaseService.initialize(firebaseConfig as FirebaseConfig)
      ...
      if (initialized) {
        ...
        if (connected) {
          ...
          error('Connection failed', 'Could not connect to Firebase')
        }
      } else {
        setConnectionStatus('error')
        error('Initialization failed', 'Could not initialize Firebase')
      }
    } catch (err) {
      setConnectionStatus('error')
      error('Firebase setup failed', (err as Error).message)
  ...
  const migrateData = async () => {
    ...
    try {
      ...
      if (migrated) {
        ...
        error('Migration failed', 'Could not migrate data to Firebase')
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">
          Simple, reliable database with real-time sync across your team
        </p>
      </div>
